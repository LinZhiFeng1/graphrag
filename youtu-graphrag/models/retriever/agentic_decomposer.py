...
class GraphQ:
    """
    GraphQ: Graph Questioning
    负责将复杂问题分解为多个子问题，以便更好地利用图谱进行检索。
    主要功能：
        读取图谱 schema
        构造提示词
        调用 LLM 进行问题分解
    ...
    def __init__(self, dataset_name, config=None):
        ...
        Args:
            ...
            config (dict, optional): 配置字典。默认为None，使用默认配置。
        """
    ...
    def prompt_format(self, schema: str, question: str) -> str:
        """
        功能: 根据图谱 schema 和问题构造提示词
        Args:
            schema (str): 图谱 schema 内容
            question (str): 问题内容
        Returns:
            str: 构建好的提示词字符串
        """
    ...
    def decompose(self, question: str, schema_path: str) -> dict:
        """
        功能: 执行完整的分解流程：读取 schema -> 构建提示词 -> 调用 LLM -> 解析响应
        Args:
            question (str): 问题内容
            schema_path (str): 图谱 schema 文件路径
        Returns:
            dict: 包含子问题和涉及类型的字典
        """
