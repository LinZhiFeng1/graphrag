...
# === 1. 加载数据 ===
# 请确保文件名与你实际存放的路径一致
file_baseline = "graph_baseline.json"
file_ours = "graph_ours.json"
...
# 绘制 Baseline 柱子
p1 = ax.bar(labels[0], n_common, width, label='共同保留关系', color=color_common, edgecolor='grey')
p2 = ax.bar(labels[0], n_base_only, width, bottom=n_common, label='基线独有关系', color=color_discarded, hatch='//', edgecolor='grey')

# 绘制 Ours 柱子
p3 = ax.bar(labels[1], n_common, width, color=color_common, edgecolor='grey')
p4 = ax.bar(labels[1], n_ours_only, width, bottom=n_common, label='拓扑增强新关系', color=color_new, hatch='xx', edgecolor='grey')
...
# 设置图表细节
ax.set_ylabel('知识图谱边数量', fontsize=12)
