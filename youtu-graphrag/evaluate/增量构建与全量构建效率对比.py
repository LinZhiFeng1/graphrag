...
# 画柱子
bars1 = plt.bar(x - width/2, full_times, width, label='Baseline (全量构建)', color='#5470c6', alpha=0.9)
bars2 = plt.bar(x + width/2, inc_times, width, label='Ours (增量构建)', color='#91cc75', alpha=0.9)

# 设置标签和标题
plt.xlabel('数据量百分比 (%)', fontsize=12)
plt.ylabel('构建时间(秒)', fontsize=12)
plt.title('效率对比: 增量构建 vs 全量构建', fontsize=14)
