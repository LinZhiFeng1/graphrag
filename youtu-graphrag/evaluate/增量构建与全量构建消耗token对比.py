...
bars1 = plt.bar(x - width/2, full_tokens, width, label='Baseline (全量构建)', color='#5470c6', alpha=0.9)
bars2 = plt.bar(x + width/2, inc_tokens, width, label='Ours (增量构建)', color='#91cc75', alpha=0.9)

# 设置标签和标题
plt.xlabel('数据百分比 (%)', fontsize=12)
plt.ylabel('Token消耗数量', fontsize=12) # Y轴改成 Token 消耗
plt.title('消耗对比: 增量构建 vs 全量构建', fontsize=14)
