...
with open('graph_baseline.json', 'r', encoding='utf-8') as f:
    data_baseline = json.load(f)
with open('graph_ours.json', 'r', encoding='utf-8') as f:
...
if G_base.has_node(TARGET_NODE) and G_ours.has_node(TARGET_NODE):
    ...
    def draw_sub(ax, G, title):
        ...
        # 画节点：增大尺寸
        nx.draw_networkx_nodes(G, pos, ax=ax, node_color=node_colors, node_size=1200, alpha=0.9, edgecolors='white')

        # 画标签：应用换行函数
        labels = {n: wrap_text(n, width=10) for n in G.nodes()}
        nx.draw_networkx_labels(G, pos, labels=labels, ax=ax, font_family=font_name, font_size=9, font_weight='bold')

        # 画边
        nx.draw_networkx_edges(G, pos, ax=ax, edge_color='gray', width=1.0, arrowsize=15,
                               connectionstyle="arc3,rad=0.1")

        # 边标签：增大字体并调整位置避免遮挡（原fontsize=6改为8）
        edge_labels = {(u, v): d['relation'] for u, v, d in G.edges(data=True)}
        nx.draw_networkx_edge_labels(
            G, pos, edge_labels=edge_labels, ax=ax,
            font_family=font_name, font_size=8, font_weight='normal',
            bbox=dict(boxstyle='round,pad=0.2', facecolor='white', alpha=0.8)  # 添加背景框避免遮挡
        )
        ...
        ax.margins(0.3)  # 增加边缘留白，防止被切
        ax.axis('off')


    draw_sub(axes[0], sub_base, f"Baseline (基线方法): {TARGET_NODE}")
